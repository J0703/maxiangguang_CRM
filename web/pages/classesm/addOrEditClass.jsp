...
<form action="${pageContext.request.contextPath}/classes/add.action?classId=${param.classId}" method="post">
    ...
    <table width="88%" border="0" class="emp_table" style="width:80%;">
        <tr>
            <td width="10%">班级名称：</td>
            <td width="20%"><input type="text" name="className" value="${param.className}"/></td>
            <td width="8%">所属类别：</td>
            <td width="62%">
                <select name="courseId" id="courseId">
                    <option value="-1">---请选择---</option>
                    <s:iterator var="course" value="courses">
                        <option value="${course.courseId}" <c:if test="${course.courseId == param.courseId}">selected</c:if>>${course.courseName}</option>
                    </s:iterator>
                </select>
            </td>
        </tr>
        <tr>
            <td>开课时间：</td>
            <td>
                <input type="text" name="beginTime" value="${param.beginTime}" readonly="readonly"
                       onfocus="c.showMoreDay=true; c.show(this);"/>
            </td>
            <td>结业时间：</td>
            <td>
                <input type="text" name="endTime" value="${param.endTime}" readonly="readonly"
                       onfocus="c.showMoreDay=true; c.show(this);"/>
            </td>
        </tr>
        <tr>
